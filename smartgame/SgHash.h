...
{
    static const std::bitset<N> mask((unsigned long long)(0xffffffffUL));
...
{
    SgHash hashcode;
    hashcode.m_code = std::bitset<N>((unsigned long long)(SgRandom::Global().Int()));
    for (int i = 1; i < (N / 32); ++i)
    {
        hashcode.m_code <<= 32;
        hashcode.m_code |= (unsigned long long)(SgRandom::Global().Int());
    }
