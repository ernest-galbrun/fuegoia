...
public:
    ...
    */
    ...
    unsigned long int Int(unsigned long range);

    /** Get a random integer in [min, max - 1] */
    unsigned long Range(unsigned long min,unsigned long max);
    ...
    */
    unsigned long PercentageThreshold(unsigned long  percentage);

    /** return true if random number SgRandom() <= threshold */
    bool RandomEvent(unsigned long threshold);
...
inline unsigned long SgRandom::Int()
{
    return m_generator();
}

inline unsigned long SgRandom::Int(unsigned long range)
{
    SG_ASSERT(range > 0);
    SG_ASSERT(static_cast<unsigned int>(range) <= SgRandom::Max());
    unsigned long i = Int() % range;
    SG_ASSERTRANGE(i, 0, range - 1);
    return i;
}

inline unsigned long SgRandom::Max()
{
    return m_generator.max();
}

inline unsigned long SgRandom::PercentageThreshold(unsigned long percentage)
{
    return (m_generator.max() / 100) * percentage;
}

inline bool SgRandom::RandomEvent(unsigned long threshold)
{
    return Int() <= threshold;
}

inline unsigned long SgRandom::Range(unsigned long min, unsigned long max)
{
