                            ...
                            const SgUctNode& child) const
{
    ...
    return GetBound(useRave, Log(float(posCount)), child);
}
    ...
    if (useRave && child.HasRaveValue())
    {
        ...
        float weight =
              ...
              / (  float(m_raveWeightParam1)
                 + float(m_raveWeightParam2) * raveCount
                );
...
float SgUctSearch::GetValueEstimateRave(const SgUctNode& child) const
{
    ...
    if (child.HasMean())
    {
        ...
        if (hasRave)
        {
            float moveCount = float(child.MoveCount());
            float raveCount = child.RaveCount();
            float weight =
                raveCount
                / (moveCount
                   * (float(m_raveWeightParam1) + float(m_raveWeightParam2) * raveCount)
                   + raveCount);
...
void SgUctSearch::PlayGame(SgUctThreadState& state, GlobalLock* lock)
{
    ...
    if (! info.m_nodes.empty() && info.m_nodes.back()->IsProven())
    {
        for (size_t i = 0; i < m_numberPlayouts; ++i)
        {
            ...
            float eval = info.m_nodes.back()->IsProvenWin() ? float(1.0) : float(0.0);
                          ...
                          SgUctEarlyAbortParam* earlyAbort)
{
    ...
    return (m_tree.Root().MoveCount() > 0) ? m_tree.Root().Mean() : float(0.5);
}
                                          ...
                                          const SgUctNode& node)
{
    ...
    float logPosCount = Log(float(posCount));
    const SgUctNode* bestChild = 0;
...
void SgUctSearch::UpdateCheckTimeInterval(double time)
{
    ...
    // m_checkTimeInterval)
    float wantedTimeDiff = (m_maxTime > 1 ? float(0.1) : float(0.1 * m_maxTime));
