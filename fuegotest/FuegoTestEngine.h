...
public:
    ...
    */
    FuegoTestEngine(GtpInputStream& in, GtpOutputStream& out,
                    ...
                    const std::string& player = "", const std::string& geNNPath = "");
...
private:
    ...
    GoPlayer* CreatePlayer(const std::string& name, boost::filesystem::path geNNPath);

    void SetPlayer(const std::string& playerId, boost::filesystem::path geNNPath);
};
