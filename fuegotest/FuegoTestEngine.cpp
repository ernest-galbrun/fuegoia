      ...
      m_safetyCommands(Board())
{
	...
	SetPlayer(player, filesystem::path(programPath).parent_path()/"Players"/geNNPath);
}
...
GoPlayer* FuegoTestEngine::CreatePlayer(const string& playerId, filesystem::path geNNPath)
{
    GoBoard& bd = Board();
    if (playerId == "")
        return 0;
	...
	if (playerId == "geNN") {
		SetNamedRules("cgos");
		return new GeNNGoPlayer(bd, geNNPath.string());
	}
		//bd.Set
    throw SgException("unknown player " + playerId);
}

void FuegoTestEngine::SetPlayer(const string& playerId, filesystem::path geNNPath)
{
	GoPlayer* player = CreatePlayer(playerId, geNNPath);
