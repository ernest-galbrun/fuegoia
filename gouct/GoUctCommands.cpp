...
void GoUctCommands::CmdParamGlobalSearch(GtpCommand& cmd)
{
    ...
    else if (cmd.NuArg() == 2)
    {
        ...
        if (name == "live_gfx")
            ...
            p.m_lengthModification = float(cmd.FloatArg(1));
        else if (name == "score_modification")
            p.m_scoreModification = float(cmd.FloatArg(1));
        else
...
void GoUctCommands::CmdParamSearch(GtpCommand& cmd)
{
    ...
    else if (cmd.NuArg() == 2)
    {
        ...
        if (name == "keep_games")
            ...
            s.SetBiasTermConstant(float(cmd.FloatArg(1)));
        else if (name == "expand_threshold")
            s.SetExpandThreshold(cmd.SizeTypeArg(1, 1));
        else if (name == "first_play_urgency")
            s.SetFirstPlayUrgency(float(cmd.FloatArg(1)));
            ...
            s.SetRaveWeightFinal(float(cmd.FloatArg(1)));
        else if (name == "rave_weight_initial")
            s.SetRaveWeightInitial(float(cmd.FloatArg(1)));
        else
            throw GtpFailure() << "unknown parameter: " << name;
    }
    else
...
SgPointSet GoUctCommands::DoFinalStatusSearch()
{
    ...
    for (GoBlockIterator it(m_bd); it; ++it)
    {
        ...
        if (! isDead && ! safe[c].Contains(*it))
        {
            ...
            const float threshold = float(0.3);
            isDead =
